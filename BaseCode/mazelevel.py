...
class MazeLevel:
    ...
    def __maze_loop(self, maze_lst, check_maze_cell):
        row_num = -1
        for row in maze_lst:
            row_num += 1
            col_num = -1
    ...
    def __check_maze_layout(self, cell, position):
        if cell == "X":  # if there are walls
            WallHidden(position,
                       [self.__game_camera, self.__wall_sprites])  # all the groups that belong to the sprite class

        if cell == "Y":
            WallVisible(position, [self.__game_camera, self.__wall_sprites])

        if cell in " PUECOSH":  # if there are floor tiles, including on the player
            self.floor = Floor(position, [self.__game_camera])

    def __check_maze_elements(self, cell, position):
        if cell == "P":  # if there is a player
            self.character = Character(position, [self.__game_camera], self.__wall_sprites, self.__powerup_sprites, self.__coin_sprites,
                                       self.__health_pot_sprites, self.__exit_sprites, self.__sword_sprites)

        elif cell == "U":
            self.powerup = Powerups(position, [self.__game_camera, self.__powerup_sprites])

        elif cell == "C":
            self.coins = Coins(position, [self.__game_camera, self.__coin_sprites])
            ...
            self.health_pot = HealthPot(position, [self.__game_camera, self.__health_pot_sprites])

        elif cell == "O":
            self.exit = Exit(position, [self.__game_camera, self.__exit_sprites])
